    ...
    <div class="zoom-image-container">
      ...
      <div class="zoom-thumbnail">
        <ul class="thumbnail-list">
          <li class="thumbnail-item" v-for="(image, index) in imageList" :key="index" @click="chooseImage(index)" :class="{show: index === showImageIndex}">
            <img class="thumbnail-img" :src="image.imageUrl" alt="">
          </li>
        </ul>
      </div>
      <van-swipe ref="zoomSwipe" :loop="true" :show-indicators="true" @change="touchChange">
        <van-swipe-item v-for="(image, index) in imageList" :key="index" :class="{zoom: index === showImageIndex && zoomState === true}">
          <img :src="image.imageUrl" :style="zoomState && offsetPercent ? `transform: translate3d(0px, ${offsetPercent}%, 0px); cursor: move;` : ''" alt="" @click="zoomImage" @mousemove="move">
        </van-swipe-item>
      </van-swipe>
